...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
  try {
     ...
     */
    const instances = await Instances.find({ shortName: shortName, status: "queued" }).catch(e => { console.error(e); throw new Error(e) });
    ...
    let queue = await sortQueue(instances).catch(e => { console.error(e); throw new Error(e) });
    ...
    if (next && next === "true") {
      ...
      const instance = await Instances.findOneAndUpdate({ shortName: shortName, id: leadingInstance.id }, { $set: { leadingInstance } }).catch(e => { console.error(e); throw new Error(e) });
      ...
      const instances = await Instances.find({ shortName: shortName }).catch(e => { console.error(e); throw new Error(e) });;
      ...
      queue = await sortQueue(instances).catch(e => { console.error(e); throw new Error(e) });
    }
