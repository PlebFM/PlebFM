    ...
    <>
      ...
      <div className="px-6 py-12 text-white relative z-50 justify-between flex flex-col space-y-6 items-center min-h-screen font-thin">
        ...
        <div className="w-2/3 mx-auto flex flex-col space-y-4 text-center">
          <Avatar firstNym={props.userProfile.firstNym} lastNym={props.userProfile.lastNym} color={props.userProfile.color} />
          <p className="text-2xl">{props.userProfile.firstNym} {props.userProfile.lastNym}</p>
        </div>

        <div className="text-3xl leading-relaxed text-center">
          <p>
            Let’s jam,<br /> <strong>{props.userProfile.firstNym} {props.userProfile.lastNym}</strong>.
          </p>
        </div>

        <Button size="large" icon={<SearchIcon />} onClick={props.setNewUserFunc}>Find a Song</Button>
      </div>
    </>
  )
}